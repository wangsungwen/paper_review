...
1. **☁️ 雲端 LLM**：
   ...
   - **記憶體隔離防護**：使用者在畫面右側輸入的任何 API Key 及模型切換，皆受 Streamlit 會話機制 (`st.session_state`) 保護。隨改隨套用，**且不會被存入伺服器硬碟**，確保在公共伺服器上多人同時使用時，不會發生金鑰外洩或設定強蹦的隱私問題。

2. **🤖 本地 LLM (僅限後端管理員配置)**：
   - 為了防護伺服器層級的 OOM (Out of Memory) 崩潰，您的本地端 Llama (GGUF) 及 Hugging Face 模型皆採用 **全網域單例快取 (Global Singleton Cache)** 機制 (`@st.cache_resource`)。
   - **改變與載入規則**：如果您是伺服器管理員，且希望更換本地模型路徑或加大 `n_ctx` 以載入更長的論文，請直接在後台修改專案目錄下的 `config.json` 檔案中的 `local` 區塊，並重新啟動伺服器 (`streamlit run app.py`)。一般外部連線的使用者無權重啟或消耗額外的顯示卡記憶體。
